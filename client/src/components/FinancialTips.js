    ...
    if (loading) {
      return (
        <div className="text-center mt-20">
          <p className="text-center pl-4 pr-4">{loadingMessage}</p>
          <div className="inline-block ml-auto mr-auto mt-4">
            <FiLoader className="animate-spin icon-spin" />
          </div>
          <Alert variant="info" className="text-center mt-4">Note: These tips are generated by AI and may not always be perfectly accurate.</Alert>
          <Alert className="text-center mt-4">This might take a few moments.</Alert>
        </div>
      );
    }
            ...
            {type !== 'section' && type !== 'highlighted' && (
              <div className={`text-base ${type === 'bold' ? 'font-bold text-blue-600' : ''} ${type === 'semiBold' ? 'font-semibold text-green-500' : ''} ${type === 'normal' ? (theme === 'dark' ? 'text-white' : 'text-gray-800') : ''}`}>
                ...
                {type === 'section' ? text : (
                  ...
                  <ul className="list-inside"><CgEditBlackPoint className='mb-3' />
